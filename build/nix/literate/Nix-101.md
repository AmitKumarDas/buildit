...
```nix
let
  buildImage = arch:                    # a function argument
    { dockerTools, callPackage }:       # some more arguments to function
...
```nix
  ...
  images = map ({ arch, pkgs }: rec {
    inherit arch;
    image = pkgs.callPackage (buildImage arch) { };       # Other Args have Same Name in nixpkgs & are Auto Passed
    tag = "${tagBase}-${arch}";
  }) crossSystems;
```
